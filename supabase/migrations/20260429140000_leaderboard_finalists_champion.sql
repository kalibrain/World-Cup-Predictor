-- Show bracket finalists and champion on the leaderboard instead of tiebreaker picks.
...
returns table (
  ...
  predicted_finalist_1 text,
  predicted_finalist_2 text,
...
begin
  ...
  scored as (
    select
      ...
      bkp.matches->'FINAL'->'slot1'->>'teamId' as predicted_finalist_1,
      bkp.matches->'FINAL'->'slot2'->>'teamId' as predicted_finalist_2,
      bkp.matches->'FINAL'->>'winnerId' as predicted_champion,
  ...
  select
    ...
    s.predicted_finalist_1,
    s.predicted_finalist_2,
    s.predicted_champion,
    s.group_points,
    s.knockout_points,
    s.total_points,
